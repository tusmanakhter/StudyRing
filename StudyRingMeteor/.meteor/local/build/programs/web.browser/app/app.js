...
if (Meteor.isClient) {                                                                                        // 46
    ...
    Template.Register.events({                                                                                // 53
        'submit form': function (event) {                                                                     // 54
            ...
            var userName = event.target.registerUsername.value;                                               // 56
            var emailVar = event.target.registerEmail.value;                                                  // 57
            var passwordVar = event.target.registerPassword.value;                                            // 58
            Accounts.createUser({                                                                             // 59
                username: userName,                                                                           // 60
                email: emailVar,                                                                              // 61
                password: passwordVar                                                                         // 62
            });                                                                                               // 59
        }                                                                                                     // 64
    });                                                                                                       // 53
    Template.Login.events({                                                                                   // 66
        'submit form': function (event) {                                                                     // 67
            event.preventDefault();                                                                           // 68
            var emailVar = event.target.loginEmail.value;                                                     // 69
            var passwordVar = event.target.loginPassword.value;                                               // 70
            Meteor.loginWithPassword(emailVar, passwordVar);                                                  // 71
            FlowRouter.go('dashboard');                                                                       // 72
        }                                                                                                     // 73
    });                                                                                                       // 66
}                                                                                                             // 75
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
Template["HomeLayout"] = new Template("Template.HomeLayout", (function() {                                    // 3
  ...
  return [ Spacebars.include(view.lookupTemplate("Header")), HTML.Raw('\n        <main>\n            <div class="billboard">\n                <h2>Study Together</h2>\n            </div>\n        </main>\n    '), Blaze.If(function() {
    ...
    return [ "\n      ", Blaze.If(function() {                                                                // 10
      return Spacebars.call(view.lookup("register"));                                                         // 11
    }, function() {                                                                                           // 12
...
Template.HomeLayout.events({                                                                                  // 1
  'click .register': function () {                                                                            // 2
    ...
    console.log("clicked register button");                                                                   // 4
  }                                                                                                           // 6
});                                                                                                           // 1
Template.HomeLayout.helpers({                                                                                 // 9
  register: function () {                                                                                     // 10
    if (Session.get('register') == null) {                                                                    // 11
      return false;                                                                                           // 12
    } else {                                                                                                  // 13
      return Session.get('register');                                                                         // 14
    }                                                                                                         // 15
  }                                                                                                           // 16
});                                                                                                           // 9
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
